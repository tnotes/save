					...
					<el-table-column label="Content" align="left" :sortable="false" :fixed="false" width="260">
						<template slot-scope="scope">
							<el-tooltip :content="'ID Post: '+scope.row.id_post" effect="dark" placement="top">
								<el-link target="_blank" :href="'http://localhost/api/save/view?id_post='+scope.row.id_post" type="primary" :underline="true" :disabled="false">
	...
	export default {
		...
		methods:{
			...
			async remove_post(data){
				await this.$axios.delete('http://localhost/api/post/'+data.id);
			...
			async UID_Like(row){
				...
				let {data} = await this.$axios.post('http://localhost/api/post/get-uid-like',{id_post:row.id_post,cookie:this.cookie});
				this.loading = false;
				this.persons = data;
			...
			async UID_Comment(row){
				...
				let {data} = await this.$axios.post('http://localhost/api/post/get-uid-comment',{id_post:row.id_post,cookie:this.cookie});
				this.loading = false;
				this.persons = data;
			},
			async UID_Share(row){
			...
			async refresh(){
				...
				let {data} = await this.$axios.get('http://localhost/api/post',{params:{where}});
				data = data.map(e=>{
