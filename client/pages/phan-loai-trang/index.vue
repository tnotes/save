...
<script>
	export default {
		...
		methods:{
			...
			async load_data(){
				let {data} = await this.$axios.get('http://localhost/api/save?limit=8&select=origin,url,createdAt&skip='+(8*(this.page - 1 )),{params:{where:this.condition}});
				this.list_link = data;
			},
			async count(){
				let {data} = await this.$axios.get('http://localhost/api/save/count',{params:{condition:JSON.stringify(this.condition)}});
				this.origins = data;
			...
			async remove(row){
				let {data} = await this.$axios.delete('http://localhost/api/save/'+row.id);
