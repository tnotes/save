...
let Num_Comment = ({id,ft_ent_identifier,reaction,cookie})=>{
    return new Promise(resolve=> {
        let option = {
            method: 'get',
...
let getIDcomment = ({id,num,cookie,fb_dtsg,ft_ent_identifier})=>{
    return new Promise(resolve=> {
        let option = {
            method: 'post',
...
module.exports = async (id,cookie)=>{
    let url = 'https://facebook.com/'+id;
    let result = [];
    let fb_dtsg_ALL = await fb_dtsg_ACTION({url,cookie});
    ...
    let nump = await Num_Comment({id,ft_ent_identifier,cookie,fb_dtsg});
    async function run (num){
        if(num < 50 ) {
            let join = await getIDcomment({id,num,cookie,fb_dtsg,ft_ent_identifier});
    ...
    if(nump < 50){
        let join = await getIDcomment({id,num:nump,cookie,fb_dtsg,ft_ent_identifier});
        return join
    }else {
